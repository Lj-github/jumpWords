  ...
  */
   ...
   */
    ...
    */
    ...
    let type = 'image/png' //图片格式，默认为 image/png
    let encoderOptions = 1//在指定图片格式为 image/jpeg 或 image/webp的情况下，可以从 0 到 1 的区间内选择图片的质量。如果超出取值范围，将会使用默认值 0.92。其他参数会被忽略。
    export function getScreenShotInWebglModele() {
        //模糊点 好像也还行  
        return document.getElementsByTagName("canvas")[0].toDataURL(type, encoderOptions)
